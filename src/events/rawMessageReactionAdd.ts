		...
		try {
			...
			switch (emoji.name) {
				...
				case '➕': {
					...
					return message.channel.send(`You are ${following.includes(guild.id) ? 'no longer' : 'now'} following this user.`).then(response => (response as Message).delete({ timeout: 5000 }));
				}
				default: return null;
			}
		} catch (error) {
