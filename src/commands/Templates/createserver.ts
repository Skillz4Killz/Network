...
export default class extends Command {
	...
	public async run(message: KlasaMessage, [guild]: [KlasaGuild]) {
		...
		const templateExists = templates.includes(guild.id);
		if (!templateExists) return message.sendMessage(`Sorry, I was not able to find ${guild.id} as a valid template. Please try again with a valid guild id that you have saved using the **.savetemplate** command.`);
		...
		try {
			...
			return message.sendMessage(`Your server has been setup using the ${guild.name} template.`);
		} catch (error) {
